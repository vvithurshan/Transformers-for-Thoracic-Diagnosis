...
# FIX: Set Matplotlib backend to 'Agg' (headless) to prevent threading/GUI errors
import matplotlib
matplotlib.use('Agg') 
...
def unfreeze_last_block_and_head(model):
    """
    Unfreeze last transformer block and classification head for ViT, DeiT, and Swin models.
    """
    # number_of_tf_block = 1
    # --- 1. Try ViT / DeiT style ---
    if hasattr(model, "blocks"):
        # blocks exist (ViT / DeiT)
    ...
    # --- 2. Try Swin style ---
        ...
        raise ValueError("Model architecture not recognized for unfreezing last block")
...
def main():
    ...
    for model_name, lr, bs in combinations:
        ...
        try:
            ...
            wandb.init(project="vit-PT-Nov-28-unfreeze-1-TF", name=exp_name, reinit=True)
            wandb.config.update({
